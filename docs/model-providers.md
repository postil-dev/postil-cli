...
Local review defaults to the embedded, tested `openai/gpt-5.6-luna` preset with no fallback chain or scorer. Reviewer requests use `low` reasoning effort and scorer-role requests use `none`. `postil models` prints the tested preset, accepted effort values, and exact override syntax. Set `REVIEW_MODEL` to override the generator and `REVIEW_MODEL_CASCADE` to add explicit fallbacks. Scoring is disabled unless `REVIEW_SCORER_MODEL` names a scorer. Hosted deployments continue to admit only complete qualified profiles. `REVIEW_SCORER_MODEL_CASCADE` accepts one qualified scorer fallback.
