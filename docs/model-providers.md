...
OpenAI-compatible requests carry the [OpenRouter reasoning configuration](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens) as `reasoning.effort`. The accepted values are `max`, `xhigh`, `high`, `medium`, `low`, `minimal`, and `none`. Scorer requests exclude returned reasoning unless reasoning is disabled. Postil does not drop or downgrade the selected effort, so custom endpoints must implement the extension.
