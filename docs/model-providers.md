...
Hosted deployments admit only complete profiles listed in the embedded `qualified-models.json` qualification manifest. A profile binds the canonical API base, provider API format, ordered generator chain, consensus width, ordered scorer chain, exact Bun evaluator runtime, qualification source commit, at least three complete runs, and SHA-256 digests for the review contract, evaluator contract, fixture set, and benchmark report. The manifest also binds the exact `config.toml` digest. Hosted configuration must match one profile exactly; an empty, degraded, or mismatched profile rejects inference.

Every nonempty manifest has a committed `qualified-models.attestation.json` bundle. CI uses [`gh attestation verify`](https://cli.github.com/manual/gh_attestation_verify) to require SLSA provenance from the exact `postil-dev/postil-cli` admission workflow and source commit, issued by GitHub OIDC through public Sigstore on a GitHub-hosted runner. The bundle authenticates the exact manifest bytes. Checksums inside the report and manifest provide integrity evidence, not producer authentication. An empty manifest carries a null qualification source, needs no bundle, and admits no model.
