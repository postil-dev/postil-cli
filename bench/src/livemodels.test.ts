...
describe("managed admission workflow", () => {
  test("pins OpenRouter and isolates candidate output from the checkout", async () => {
    ...
    expect(release).not.toContain("baseline-us");
    expect(release).not.toContain("provisional-models-us.json");
    expect(release).toContain("bench/baseline.json");
    expect(release).toContain("--baseline baseline.json");
    expect(release).toContain("--bundle bench/baseline.attestation.json");
