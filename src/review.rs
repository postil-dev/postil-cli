...
fn generate_finding_ids(findings: &mut [Finding], head_sha: Option<&str>) {
    ...
    for finding in findings.iter_mut() {
        ...
        let prehash_key = format!("{}{}{}{}", head_sha, finding.kind.as_str(), normalized_path, normalized_line);
        let duplicate_index = id_map.entry(prehash_key).and_modify(|count| *count += 1).or_insert(0);
        
        // Build the hash input
        let hash_input = format!("{}\x00{}\x00{}\x00{}\x00{}\x00{}", 
            head_sha, finding.kind.as_str(), normalized_path, normalized_line, normalized_title, duplicate_index);
        
        // Generate SHA256 hash
        let mut hasher = Sha256::new();
        hasher.update(hash_input.as_bytes());
        let result = hasher.finalize();
        let hex_id = result.iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
async fn review_diff(cfg: &Config, args: &ReviewArgs, input: ReviewInput<'_>) -> Result<Envelope> {
    ...
    Ok(Envelope {
        ...
        gate: Gate {
            ...
            block_on_kinds: cfg.block_on_kinds.iter().map(|k| k.as_str().to_string()).collect(),
        },
        model_used,
        usage,
